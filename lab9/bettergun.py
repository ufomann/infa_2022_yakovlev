...
class Gun:
    def __init__(self, screen):
        ...
        self.__ang = 1
        self.__color = GREY
        self.__coords = np.array([WIDTH / 2, HEIGHT / 2])
        self.__image = pygame.image.load("graphics/redshipLowerSpd.png").convert_alpha()
        SIZE = np.array([self.__image.get_width(), self.__image.get_height()])
        self.__image = pygame.transform.scale(self.__image, SIZE * 10)
    ...
    def draw(self):
        ...
        self.__rot %= 360
        img = self.__image
        temp = pygame.transform.rotate(img, self.__rot)
        #image = pygame.transform.rotate(image, 90)
        screen.blit(temp, (self.__coords[0] -temp.get_width() / 2,self.__coords[1] -temp.get_height() / 2))
